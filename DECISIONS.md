...
### ADR-018: Phase 2.7 Unified Evidence & Provenance Layer Architecture
* **Status**: ACCEPTED
* **Context**: Every data-driven answer, risk calculation, and UI artifact in SAMUDRA AI must explain what data was used, where it came from, when it was retrieved, what observation/forecast window it represents, what authority classification applies, how fresh it is, and what limitations govern its use. All provenance metadata must originate from deterministic application code; LLMs are strictly prohibited from fabricating provider names, dataset IDs, timestamps, coordinates, or escalating authority classifications. Parallel LangGraph data collection nodes must be able to write evidence to state without race conditions.
* **Decision**: Create a dedicated evidence and provenance engine (`tools/evidence_service.py`) defining standardized enumerations (`AuthorityClass`, `DataClass`, `EvidenceStatus`, `FreshnessStatus`, `SourceType`), evidence record builders (`create_evidence_record`), deterministic freshness age evaluators (`calculate_freshness`), and evidence completeness aggregators (`aggregate_evidence`). Extend `SamudraState` (`state/schema.py`) with `evidence: Annotated[list[dict[str, Any]], operator.add]` to enable race-condition free parallel evidence accumulation during LangGraph collector execution. Update all data nodes (`data_weather.py`, `data_ocean.py`, `data_fishery.py`, `data_tide_hazard.py`, `data_geofence.py`, `data_route.py`) to emit standardized evidence records into state. Enforce explicit `UNAVAILABLE` evidence records for unmapped layers (`mpa_gis_boundaries`, `naval_defence_gis_zones`) with `OFFICIAL_GOVERNMENT_REQUIRED` authority class. Update `risk_assessment_node` (`graph/nodes/risk.py`) to compute aggregated `evidence_summary` completeness scores. Expose evidence metadata across UI artifacts (`tools/artifact_factory.py`) and translation outputs (`graph/nodes/translate_out.py`).
